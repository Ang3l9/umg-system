...
function NotificacionesCtList() {
    ...
    const fetchNotificaciones = async () => {
        try {
            const response = await api.get('/api/v1/notificaciones-ct');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                await api.put(`/api/v1/notificaciones-ct/${editId}`, payload, {
                    headers: { 'Content-Type': 'application/json' }
                });
            } else {
                await api.post('/api/v1/notificaciones-ct', payload, {
                    headers: { 'Content-Type': 'application/json' }
                });
            }
            setFormData({
    ...
    const handleDelete = async (id) => {
        try {
            await api.delete(`/api/v1/notificaciones-ct/${id}`);
            setError(null);
            fetchNotificaciones();
        } catch (err) {
